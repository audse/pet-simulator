class_name GuideMapHandleGroup extends HandleGroup
...
export(Mode) var mode = TileMap.MODE_ISOMETRIC
...
func _init(base_map_value:TileMap=base_map, is_autotile_value:bool=is_autotile) -> void:
	self.base_map = base_map_value
...
func handle_drag_released(from_pos:Vector2, to_pos:Vector2, handle:Handle) -> void:
	if NodeRef.is_node_valid(guide.tile_map, TileMap):
		guide.tile_map.clear()
		from_pos = handle.base_map.world_to_map(from_pos)
		to_pos = handle.base_map.world_to_map(to_pos)
...
func set_base_map(new_base_map:TileMap) -> void:
	...
	if NodeRef.is_node_valid(new_base_map, TileMap):
		for handle in get_children():
			if handle is GuideMapHandle:
				
				handle.base_map = new_base_map
				
		guide.tile_map.cell_size = new_base_map.cell_size
		guide.tile_map.mode = new_base_map.mode
