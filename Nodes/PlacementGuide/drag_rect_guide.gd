...
export (Vector2) var grid_size:Vector2 = Vector2(48, 24)
...
onready var nodes:Dictionary = {
	...
	hand_icon = $GuideRect/HandIcon,
	buffer = $GuideRect/BackBufferCopy,
	tween = $Tween,
}
...
func set_area_size(new_size:Vector2) -> void:
	if nodes:
		...
		nodes.drag.TouchAreaShape.position = -(new_size / 2)
