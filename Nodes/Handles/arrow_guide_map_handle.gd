...
#		var sprite_offset:Vector2 = get_texture_size() / 2
		var furthest:int = base_map.get_furthest_cell_value(direction)
		var rect:Vector2 = base_map.get_used_rect().size
		var map_offset:Vector2 = Vector2.ZERO
		
		var target_position:Vector2 = base_map.get_midpoint_cell()
		...
		var target_cell:Vector2 = (target_position + map_offset) * 2
		
		""" IDK about this part """
		var half_map:TileMap = TileMap.new()
		half_map.mode = base_map.mode
		half_map.cell_size = base_map.cell_size
		...
		return half_map.map_to_world(target_position + map_offset)
	return Vector2.ZERO
